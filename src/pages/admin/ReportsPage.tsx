      ...
      <div className="max-w-6xl mx-auto p-4 lg:p-8">
          ...
          {selectedProgram && (
            <label className="flex items-center gap-2 cursor-pointer px-3"
              ...
              <input
                type="checkbox"
                checked={onlyAtRisk}
                onChange={(e) => handleAtRiskChange(e.target.checked)}
                className="w-4 h-4 rounded"
                style={{ accentColor: '#C0271E' }}
              />
        ...
        {report && !loading && (
          <div className="flex flex-wrap gap-3 mb-5">
            {[
              { label: 'Total', value: report.total, accent: '#1A3A6B', bg: '#E8EEF8' },
              { label: 'Certificados', value: report.certified_count, accent: '#C9942A', bg: '#FDF5E0' },
              { label: 'En riesgo', value: report.at_risk_count, accent: '#C0271E', bg: '#FDECEA' },
        ...
        {!selectedProgram && (
          <div className="flex flex-col items-center justify-center py-24 gap-3">
            ...
            <p className="text-base font-semibold" style={{ color: '#8E97AE' }}>
              Selecciona un programa para ver el reporte
            </p>
          </div>
        )}

        {selectedProgram && loading && (
          <div className="text-center py-12" style={{ color: '#8E97AE' }}>Cargando reporte…</div>
        )}

        {selectedProgram && error && (
          <div className="text-center py-12 font-semibold" style={{ color: '#C0271E' }}>{error}</div>
        )}
              ...
              <tbody>
                {displayed.map((s, i) => (
                  <tr
                    key={s.id}
                    style={{
                      background: s.status === 'en_riesgo'
                        ? 'rgba(192,39,30,0.04)'
                        : s.status === 'certificado'
                        ? 'rgba(201,148,42,0.04)'
                        : '#FFFFFF',
